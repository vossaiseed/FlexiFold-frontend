...
import { LayoutDashboard, Users, Bell, User, ChevronUp, LogOut, X } from "lucide-react";
import { logout as logoutAction } from "../../../redux/features/auth/authSlice";
import api from "../../../redux/services/api";

export const salesNav = [
  { label: "Dashboard", icon: LayoutDashboard, to: "/sales/dashboard" },
  { label: "My Leads", icon: Users, to: "/sales/leads" },
  { label: "Notifications", icon: Bell, to: "/sales/notifications" },
  { label: "Profile", icon: User, to: "/sales/profile" },
];
...
function SidebarBody({ onNavClick }) {
  ...
  const currentUser = {
    name: meta.name || "Sales Team",
    role: "Sales Team",
    initial: (meta.name?.[0] || "S").toUpperCase(),
  };
  ...
  return (
    <>
      <div className="flex items-center gap-3 border-b border-slate-100 px-5 py-5">
        <div className="flex h-9 w-9 items-center justify-center rounded-lg bg-emerald-500 text-base font-bold text-white">S</div>
        <div>
      ...
      <nav className="flex-1 space-y-1 overflow-y-auto p-3">
        {salesNav.map(({ label, icon: Icon, to }) => (
          <NavLink
            key={label}
            to={to}
            onClick={onNavClick}
