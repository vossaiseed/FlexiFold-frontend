...
export const telecallerNav = [
  ...
  { label: "My Leads", icon: Users, to: "/telecaller/leads" },
  { label: "Call Logs", icon: PhoneCall, to: "/telecaller/call-logs" },
  { label: "Follow-ups", icon: CalendarClock, to: "/telecaller/follow-ups" },
  { label: "Notifications", icon: Bell, to: "/telecaller/notifications" },
  ...
  return (
    <>
      ...
      <div className="flex items-center gap-3 border-b border-slate-100 px-5 py-5">
        <div className="flex h-9 w-9 items-center justify-center rounded-lg bg-emerald-500 text-base font-bold text-white">T</div>
        <div>
          <p className="text-sm font-bold text-slate-900">Telecaller</p>
      ...
      <nav className="flex-1 space-y-1 overflow-y-auto p-3">
        {telecallerNav.map(({ label, icon: Icon, to }) => (
          <NavLink
            key={label}
            to={to}
            onClick={onNavClick}
...
export default function TellecallerSidebar({ open, onClose }) {
  return (
    <>
      {/* Desktop sidebar */}
      ...
      {open && (
        <div className="fixed inset-0 z-50 lg:hidden">
          ...
          <aside className="relative flex h-full w-64 flex-col bg-white shadow-xl">
            <button onClick={onClose} className="absolute right-3 top-4 rounded-lg p-1.5 text-slate-400 transition hover:bg-slate-100">
              <X className="h-5 w-5" />
            </button>
